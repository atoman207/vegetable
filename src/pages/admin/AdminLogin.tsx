...
const AdminLogin = () => {
  ...
  // Signed in but not an admin → block with a clear message + sign-out button.
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Client-side guard — reject non-admin emails before hitting Supabase.
    ...
    try {
      ...
      // Always use the same generic message so we don't leak whether the
      // email exists. Matches how real admin portals behave.
      setError("管理者のメールアドレスまたはパスワードが正しくありません。");
    } finally {
      setSubmitting(false);
    }

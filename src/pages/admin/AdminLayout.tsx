...
const sidebarItems = [
  { to: "/admin", label: "ダッシュボード", en: "Dashboard", icon: LayoutDashboard, end: true },
  { to: "/admin/producers", label: "生産者", en: "Producers", icon: Users },
  { to: "/admin/news", label: "ニュース", en: "News", icon: Newspaper },
];
    ...
    <div className="min-h-screen bg-muted/40 flex">
      ...
      <aside className="hidden lg:flex w-64 bg-foreground text-primary-foreground flex-col relative overflow-hidden">
        ...
        <nav className="relative flex-1 p-4 space-y-1">
          {sidebarItems.map(({ to, label, en, icon: Icon, end }) => (
            <NavLink
              key={to}
              to={to}
              end={end}
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive ? "bg-sun/15 text-sun" : "text-primary-foreground/75 hover:bg-primary-foreground/5"
                  )
                }
              >
